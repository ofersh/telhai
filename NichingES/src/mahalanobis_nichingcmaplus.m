%Dynamic ES Niching with (1+lambda)-CMA - with Mahalanobis distance metric.
% -------------------------------------------------------------------------------------
...
% -------------------------------------------------------------------------------------
function [X,mpr_q] = mahalanobis_nichingcmaplus(strfitnessfct,N,X_a,X_b,q,q_eff,rho,...
...
%    Boundary Conditions: May be treated here
    ...
    [DPS,pop_niche] = DPI (Y(:,:),B,D,P,(lambda+1)*q_eff,q,rho);

    %(1+lambda) Selection for each niche
    FitP = inf*ones(1,q_eff);
    XP = zeros(N,q_eff);
    for i=1:q,
        %step(i,gen+1) = sigma{i};
        j=DPS(1,i);
        if (j~=0)
            parent = P(1,j); %the original parent!
            XP(:,i) = Y(:,j);
            FitP(1,i) = arfitness(1,j);
            lsucc = (arfitness(1,j)<arP(1,parent)); %Recent Mutation Success
            ...
            FitP(1,i) = feval (strfitnessfct,XP(:,i)',bnf);
        end
    end
    if (mod(gen,kappa)==0)
        for i=q+1:q_eff,
